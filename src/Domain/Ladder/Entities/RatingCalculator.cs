...
namespace Warhammer.Domain.Ladder.Entities
{
    public class RatingCalculator
    {
        ...
        {
            this._tau = DefaultTau;
            this._defaultVolatility = DefaultVolatility;
        }
        ...
        {
            this._defaultVolatility = initVolatility;
            this._tau = tau;
        }
        ...
        /// <param name="results"></param>
        private void CalculateNewRating(Player player, IList<Result> results)
        {
            var phi = player.GetGlicko2RatingDeviation();
            var sigma = player.GetVolatility();
            var a = Math.Log(Math.Pow(sigma, 2));
            var delta = this.Delta(player, results);
            ...
            {
                double k = 1;
                B = a - (k*Math.Abs(this._tau));

                while (F(B, delta, phi, v, a, this._tau) < 0)
                {
                    k++;
                    B = a - (k*Math.Abs(this._tau));
                }
            ...
            // step 5.3
            var fA = F(A, delta, phi, v, a, this._tau);
            var fB = F(B, delta, phi, v, a, this._tau);

            // step 5.4
            while (Math.Abs(B - A) > ConvergenceTolerance)
            {
                var C = A + (((A - B)*fA)/(fB - fA));
                var fC = F(C, delta, phi, v, a, this._tau);
        ...
        /// <param name="results"></param>
        /// <returns></returns>
        private static double V(Player player, IEnumerable<Result> results)
        {
            var v = 0.0;
        ...
        /// <param name="results"></param>
        /// <returns></returns>
        private double Delta(Player player, IList<Result> results)
        {
        ...
        private static double OutcomeBasedRating(Player player, IEnumerable<Result> results)
        {
        ...
        {
            return this._defaultVolatility;
        }
